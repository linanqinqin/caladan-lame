...
static void jmp_thread(thread_t *th)
{
	...
	log_info("[LAME][sched ON]: uthread %p started (scheduled on kthread %d)", 
		  th, kthread_idx(myk()));
	/* end */
...
static void jmp_thread_direct(thread_t *oldth, thread_t *newth)
{
	...
	log_info("[LAME][sched ON]: direct switch: uthread %p scheduled on kthread %d", 
...
static __noinline void schedule(void)
{
	...
	if (likely(th != NULL)) {
		...
		log_info("[LAME][sched OFF]: uthread %p descheduled from kthread %d", 
			  th, kthread_idx(l));
		/* end */
...
again:
	...
	store_release(&th->cur_kthread, kthread_idx(l));
...
static __always_inline void enter_schedule(thread_t *curth)
{
	...
	log_info("[LAME][sched OFF]: uthread %p entering scheduler (descheduled from kthread %d)", 
		  curth, kthread_idx(k));
	/* end */	
...
void thread_finish_yield(void)
{
	...
	log_info("[LAME][sched OFF]: uthread %p ceding (descheduled from kthread %d)", 
		  curth, kthread_idx(k));
	/* end */

...
void lame_print_tsc_counters(void)
{
	...
	for (i = 0; i < maxks; i++) {
		...
		if (!k)
			continue;
		log_warn("[LAME][TSC][kthread:%u] total_cycles=%lu total_lames=%lu avg_cycles=%lu", i,
		         k->lame_bundle.total_cycles, k->lame_bundle.total_lames, k->lame_bundle.total_cycles / k->lame_bundle.total_lames);
	}
}
#endif
/* end */